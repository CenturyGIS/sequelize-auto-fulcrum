  ...
  function formFound (error, response) {
    ...
    _.forEach(self.form.allElements, function (el) {
      ...
      if (el.isRecordLinkElement && el.allowMultiple) {
        var parent = self._getParent(el);
        var parentTableName = parent.dataName || self.parent;
...
AutoFulcrum.prototype.run = function (callback) {
  ...
  this.build(function () {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      var relations = _.reduce(self.tables[table], function (rels, el) {
        if (el.isRepeatableElement) {
          rels.push(el.dataName);
        }
        else if (el.isRecordLinkElement && el.allowMultiple) {
          var parent = self._getParent(el);
          var parentDataName = parent.dataName || self.parent;
      ...
      if (relations.length) {
        ...
        _.each(relations, function (related) {
          text[table] += spaces + spaces + spaces + spaces + capitalized + '.hasMany(models.' + related + ', {\n';
