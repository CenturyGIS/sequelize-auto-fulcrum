  ...
  this.build(function () {
    ...
    async.each(
      ...
      function (table, _callback) {
        ...
        text[table] +=
          spaces + spaces + spaces + 'type: DataTypes.STRING(100),\n';
        ...
        if (table === self.parent) {
          if (self.form.statusField.isEnabled) {
            text[table] +=
              spaces +
              spaces +
              self.form.statusField.dataName +
              ': DataTypes.TEXT,\n';
          }
          if (self.form.isProjectEnabled) {
            text[table] +=
              spaces + spaces + 'project_fulcrum_id: DataTypes.TEXT,\n';
          }

          text[table] += spaces + spaces + 'version: DataTypes.INTEGER,\n';
          text[table] +=
            spaces + spaces + 'client_created_at: DataTypes.DATE,\n';
          text[table] +=
            spaces + spaces + 'client_updated_at: DataTypes.DATE,\n';
          ...
          text[table] +=
            spaces + spaces + spaces + 'type: DataTypes.STRING(100),\n';
          text[table] +=
            spaces + spaces + spaces + "comment: 'Fulcrum Record ID',\n";
          text[table] += spaces + spaces + '},\n';
        }

        if (self.geoEnabled[table]) {
          text[table] +=
            spaces + spaces + "the_geom: 'geometry(Point,4326)',\n";
          ...
          text[table] +=
            spaces + spaces + 'horizontal_accuracy: DataTypes.DECIMAL,\n';
          text[table] +=
            spaces + spaces + 'vertical_accuracy: DataTypes.DECIMAL,\n';
          text[table] +=
            spaces + spaces + 'created_location: DataTypes.JSON,\n';
          text[table] +=
            spaces + spaces + 'updated_location: DataTypes.JSON,\n';
        ...
        _.each(fields, function (field) {
          ...
          if (
            field.isRepeatableElement ||
            (field.isRecordLinkElement && field.allowMultiple)
          ) {
            return;
          }
          ...
          if (
            field.isPhotoElement ||
            field.isAudioElement ||
            field.isVideoElement
          ) {
        ...
        if (relations.length) {
          _.each(relations, function (related) {
            text[table] +=
              ...
              '.hasMany(models.' +
              related +
              ", { foreignKey: 'fulcrum_parent_id', onDelete: 'CASCADE' }); \n";
          });
        }
      ...
      function () {
        ...
        async.each(
          self.recordLinked,
          function (dataName, _callback) {
            let capitalized = _.startCase(_.toLower(dataName)).replace(
              /\s+/g,
              ''
            );
            ...
            text[dataName] +=
              spaces + spaces + spaces + 'type: DataTypes.STRING(100),\n';
            text[dataName] +=
              spaces + spaces + spaces + "comment: 'Fulcrum ID',\n";
            ...
            text[dataName] +=
              spaces + spaces + spaces + 'type: DataTypes.STRING(100),\n';
            text[dataName] +=
              spaces + spaces + spaces + "comment: 'Fulcrum Parent ID',\n";
            text[dataName] += spaces + spaces + '},\n';
            ...
            text[dataName] +=
              spaces + spaces + "tableName: '" + dataName + "',\n";
...
AutoFulcrum.prototype.write = function (attributes, callback) {
  ...
  function createFile(table, _callback) {
    fs.writeFile(
      path.resolve(path.join(self.options.directory, table + '.js')),
      attributes[table],
      _callback
    );
  }
