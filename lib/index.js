  ...
  this.build(function () {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      if (table === self.parent) {
        ...
        text[table] += spaces + spaces + 'created_location: DataTypes.TEXT,\n';
        text[table] += spaces + spaces + 'updated_location: DataTypes.TEXT,\n';
      ...
      _.each(fields, function (field) {
        ...
        text[table] += spaces + spaces + field.dataName + ': {\n';

        text[table] += spaces + spaces + spaces + 'type: DataTypes.TEXT,\n';

        var label = (field.label || '').trim().replace(/\'/, '\\\'');
        text[table] += spaces + spaces + spaces + 'comment: \'' + label + '\',\n';
      ...
      _.each(_.keys(self.options.additional), function (addl) {
        text[table] += spaces + spaces + addl + ': \'' + self.options.additional[addl] + '\',\n';
      });
...
AutoFulcrum.prototype.write = function (attributes, callback) {
  ...
  fs.mkdirSync(path.resolve(self.options.directory));
